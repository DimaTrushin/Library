...
template<class TData>
class CObservableData : protected CStorage<TData>,
                        protected CObservable<TData> {
  ...
  CObservableData(TArgs&&... args)
      : CStorageBase(std::forward<TArgs>(args)...),
        CObservableBase([&Data = CStorageBase::Data_]() ->
                        typename CSource<TData>::CGetType { return Data; }) {
  }
...
public:
  ...
  CObserverCold(T&& OnNotify)
      : CBase(CBase::doNothing, std::forward<T>(OnNotify), CBase::doNothing) {
  }
};
...
public:
  ...
  CObserverColdStrict(T&& OnNotify)
      : CBase([OnNotify](typename CBase::CGetType optData) {
          if (optData.has_value())
            OnNotify(*optData);
        }) {
  }
...
public:
  ...
  CHotActiveInput(const T& Action)
      : CBase(
            ...
            [this](typename CBase::CGetType) { deactivate(); }) {
  }
